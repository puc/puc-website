...
document.addEventListener('DOMContentLoaded', function () {
    var calendarToggle = document.querySelectorAll('a.js-calendar-view'),
        calendars = document.querySelectorAll('.js-calendar');

    if (calendarToggle) {
        calendarToggle.forEach(function (toggle) {
            toggle.addEventListener('click', function (e) {
                toggle.closest('ul').querySelectorAll('li').forEach(function (parent) {
                    parent.classList.remove('is-active');
                });

                toggle.closest('li').classList.add('is-active');

                calendars.forEach(function (calendar) {
                    if (calendar.dataset.type == toggle.dataset.view) {
...
$document.ready(function () {
    $('a.js-dropdown-toggle').on('click', function (e) {
        var menu = $(this).next('div.js-dropdown-menu');

        $('div.js-dropdown-menu').not(menu).removeClass('is-active');
...
$document.on('mouseup touchend', function (e) {
    var container = $('div.js-dropdown-menu, a.js-dropdown-toggle');
...
$document.ready(function () {
    $('.js-gallery, .uc, .is-video').foobox({
        ...
        error: {
            ...
            src: '".SITEURL."i/layout/foobox/no-img.jpg'
        },
        selector: 'a[href]',
        debug: false
    });
...
(function ($, window, console, undefined) {
  ...
  */
   ...
   */
		isMultipart: function isMultipart(name, separator) {
   ...
   */
		merge: function merge(base, object1, objectN) {
		...
		/** @private */
		_merge: function _merge(base, changes) {
			var prop;
			for (prop in changes) {
   ...
   */
		register: function register(name, obj, defaults) {
   ...
   */
		load: function load(name, instance) {
			var loaded = [],
			    registered,
			    i,
			    arr = FooBox.objects.ensure(name);
   ...
   */
		call: function call(name, instance, method, arg1, argN) {
			var args = Array.prototype.slice.call(arguments),
			    obj;
			name = args.shift();
			instance = args.shift();
   ...
   * @returns {Object}
   */
		get: function get(name, instance, where) {
			var i,
			    arr = instance.objects[name];
			if (!arr || !$.isFunction(where)) {
				return null;
			}
			for (i = 0; i < arr.length; i++) {
				if (where(arr[i])) {
   ...
   */
		validate: function validate(obj) {
			if (!$.isFunction(obj)) {
				Debug.error('Expected type "function", received type "{0}".', typeof obj === 'undefined' ? 'undefined' : _typeof(obj));
				return false;
			}
			return true;
		},
		register: function register(obj, defaults) {
		...
		call: function call(instance, method, arg1, argN) {
			var args = Array.prototype.slice.call(arguments);
   ...
   * @returns {Object}
   */
		get: function get(instance, type) {
			return FooBox.objects.get(FooBox.handlers._ns, instance, function (item) {
  ...
  */
   ...
   */
    ...
    */
			options: function options(_options) {
				var display;
				_this.element.removeClass().addClass('fbx-modal').addClass(FooBox.format('fbx-{0}', _this.FooBox.id)).addClass(_this.FooBox.element.data('style') || _options.style).addClass(_this.FooBox.element.data('theme') || _options.theme).addClass(_this.FooBox.element.data('modal-class')).addClass(_options.modalClass);
				...
				display = _options.affiliate.enabled ? '' : 'none';
				_this.element.find('.fbx-credit').css('display', display);
				if (_options.affiliate.enabled) {
					_this.element.find('.fbx-credit > a').attr('href', _options.affiliate.url);
					_this.element.find('.fbx-credit > a > em').text(_options.affiliate.prefix);
				}

				display = _options.showCount && _this.FooBox.items.multiple() ? '' : 'none';
				_this.element.find('.fbx-count').css('display', display);

				if (!_options.showButtons || !_this.FooBox.items.multiple()) {
   ...
   */
		this.show = function (first) {
			...
			if (!_this.element.hasClass('fbx-show')) {
				_this.prioritize();
				_this.element.addClass('fbx-loading').find('.fbx-inner').css({ 'width': '100px', 'height': '100px', 'margin-top': '-50px', 'margin-left': '-50px' });
			}
			...
			function handleError(err) {
				...
				Debug.error(err);
				if (_this.FooBox.raise('foobox.onError', { 'error': err }).isDefaultPrevented()) {
					return;
				}
			...
			if (_this.FooBox.raise('foobox.beforeLoad', { 'item': item }).isDefaultPrevented()) {
				return;
			}
   ...
   */
		this.resize = function (size, success, error) {
			try {
				...
				if (size.height * ratio > ms.height) {
					ratio = ms.height / size.height;
				}
				...
				if (!cs.equalTo(size)) {
					// If the size has changed resize it.
   ...
   */
		this.overflow = function (item, size) {
			if (item.overflow == true && (size.width < item.width * 0.8 || size.height < item.height * 0.8) || item.overflow == true && item.proportion == false && (size.width < item.width || size.height < item.height)) {
   ...
   */
		this.init = function (element, options) {
			...
			if (_this.items.array.length > 0) {
				...
				// self cleanup, no items, no FooBox
				_this.destroy();
			}
		};
   ...
   */
    ...
    */
     ...
     */
				...
				/** @private */
				_direction: function _direction(previous, current, max) {
    ...
    */
			new_array: function new_array(reinit) {
				reinit = reinit || false;
				var item,
				    e,
				    index = 0,
    ...
    */
			rel: function rel() {
				...
				if (typeof rel == 'string') {
					$('[rel="' + rel + '"]').not(_this.options.excludes).not(_this.element).unbind('click.item').filter(function () {
						return _this.items.add(this);
					}).bind('click.item', _this.items.clicked);
				}
			},
    ...
    */
			indexOf: function indexOf(item, prop) {
				if (!item) {
					return -1;
				}
				prop = prop || 'url';
     ...
     */
				first: function first(item) {
					if (!_this.items.array || _this.items.array.length == 0) {
						return false;
					}
    ...
    */
			error: function error(index) {
				...
				var handler = FooBox.handlers.get(_this, 'html'),
				    $element,
				    error,
				    isSelector = false;
				if (handler == null) {
					var ErrHandler = function ErrHandler(fbx) {
						var _h = this;
						_h.FooBox = fbx;
						_h.type = 'error';
						_h.handles = function () {
  ...
  */
	FooBox.open = function (arg) {
		...
		if (fbx == null || !(fbx.modal instanceof FooBox.Modal)) {
			return null;
		}
		fbx.modal.show(true);
		return fbx;
	...
	FooBox.resize = function (width, height) {
		if (width || height) {
			...
			for (i = 0; i < FooBox.instances.length; i++) {
				fbx = FooBox.instances[i];
				if (fbx == null || !(fbx.modal instanceof FooBox.Modal) || !FooBox.isjQuery(fbx.modal.element) || !fbx.modal.element.hasClass('fbx-show')) {
					continue;
				}
...
(function ($, FooBox) {
  ...
  */
   ...
   */
    ...
    */
			unbind: function unbind(all) {
				...
				if (_this.FooBox.modal instanceof FooBox.Modal && FooBox.isjQuery(_this.FooBox.modal.element)) {
					_this.FooBox.modal.element.undelegate('mouseenter.captions mouseleave.captions').find('.fbx-item-current, .fbx-item-next').unbind('click.captions');
				}
			},
			/**
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			closeOverlays: function closeOverlays(e) {
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			setupHtml: function setupHtml(e) {
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			setupOptions: function setupOptions(e) {
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			parseItem: function parseItem(e) {
		...
		this.update = function () {
			...
			$caption.html(item.caption).find('a[href^="#"]').filter(function () {
				var identifier = $(this).attr('href'),
				    target = $(identifier),
				    fbx = target.data('fbx_instance') || target.data('fbx_p_instance');
...
(function ($, FooBox) {
  ...
  */
   ...
   */
    ...
    */
			get: function get() {
				if (location.hash.indexOf(FooBox.hashBang + _this.FooBox.options.deeplinking.prefix) === -1) {
					return null;
				}
  ...
  */
   ...
   */
		...
		this.handlers = {
			...
			foobox_image_onload: function foobox_image_onload(e) {
				if (e.fb.item.type == 'image') {
					_this.raise('foobox_image_onload', { thumb: { jq: e.fb.item.element, target: e.fb.item.url } });
				}
			},
			foobox_image_custom_caption: function foobox_image_custom_caption(e) {
...
(function ($, FooBox) {
  ...
  */
   ...
   */
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			afterResize: function afterResize(e) {
    ...
    */
			beforeClose: function beforeClose(e) {
				...
				e.fb.modal.one(FooBox.browser.transitionEnd(), function () {
					...
					if (e.fb.options.hideScrollbars) {
						$body.css('overflow', '');
					}
    ...
    */
			onKeydown: function onKeydown(e) {
				var modal = _this.FooBox.modal,
				    options = _this.FooBox.options.keyboard;
...
(function ($, FooBox) {
  ...
  */
   ...
   */
    ...
    */
			resize: function resize() {
				_this.timers.resize.start(function () {
					_this.style();
					var item = _this.FooBox.items.current();
					if (!_this.FooBox.modal.element.has('fbx-show') || item == null) {
						return;
					}
    ...
    */
			ios7beforeShow: function ios7beforeShow() {
   ...
   */
    ...
    */
			unbind: function unbind(all) {
				...
				if (_this.FooBox.modal instanceof FooBox.Modal && FooBox.isjQuery(_this.FooBox.modal.element)) {
					_this.FooBox.modal.element.undelegate('click.slideshow').unbind('mousemove.foobox').find('.fbx-play, .fbx-pause').unbind('mouseenter.foobox mouseleave.foobox');
				}
			},
			/**
    * Handles the foobox.initialized event binding the various events needed by this addon to function.
    ...
    */
			setupHtml: function setupHtml(e) {
				...
				e.fb.modal.find('.fbx-inner').append('<div class="fbx-progress"></div>').append('<a href="#playpause" class="fbx-play fbx-btn-transition fbx-btn-shadow"></a>');
			},
			/**
    * Handles the foobox.setupOptions event allowing this addon to set its initial starting state.
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			fullscreenEnabled: function fullscreenEnabled(e) {
				if (e.fb.modal.hasClass('fbx-fullscreen-mode') && e.fb.modal.hasClass('fbx-playpause-center')) {
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			fullscreenDisabled: function fullscreenDisabled(e) {
				if (!e.fb.modal.hasClass('fbx-fullscreen-mode') && e.fb.modal.hasClass('fbx-playpause-center')) {
    ...
    * @param {jQuery.Event} e - A standard jQuery.Event object.
    */
			playClicked: function playClicked(e) {
				e.preventDefault();
				e.stopPropagation();
				_this.start();
			},
    ...
    * @param {jQuery.Event} e - A standard jQuery.Event object.
    */
			pauseClicked: function pauseClicked(e) {
				e.preventDefault();
				e.stopPropagation();
				_this.pause();
			},
  ...
  */
   ...
   */
    ...
    */
			unbind: function unbind(all) {
				...
				if (_this.FooBox.modal instanceof FooBox.Modal && FooBox.isjQuery(_this.FooBox.modal.element)) {
					_this.FooBox.modal.element.undelegate('mouseenter.social mouseleave.social').find('.fbx-item-current, .fbx-item-next').unbind('click.social');
				}
			},
			/**
    * Handles the foobox.initialized event binding the various events needed by this addon to function.
    ...
    */
			initialized: function initialized(e) {
				...
				if (e.fb.options.social.enabled === true) {
					...
					}).end().find('.fbx-social-toggle').unbind('click.social').bind('click.social', function (e) {
						e.preventDefault();
						e.stopPropagation();
						_this.handlers.toggleSocial();
					});
    ...
    */
			afterLoad: function afterLoad(e) {
				...
				if (e.fb.options.social.onlyShowOnHover == true || e.fb.options.social.onlyShowOnClick == true) {
					return;
				}
				_this.show();
			},
   ...
   */
		...
		this.parseUrl = function (link, item) {
			var url = link.url,
			    current = this.fixUrl(item.share_url ? item.share_url : _this.FooBox.options.social.original ? _this.fullUrl(item.url) : location.href);
   ...
   */
    ...
    */
			onTouchStart: function onTouchStart(e) {
				...
				if (touches.length != 1 || !_this.FooBox.items.multiple()) {
					return;
				}
...
(function ($, FooBox) {
  ...
  */
   ...
   */
    ...
    * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
    */
			onCreateCaption: function onCreateCaption(e) {
				...
				if (cont.hasClass('gallery')) {
					if (opt.captions.overrideTitle === false) {
						title = el.parents(".gallery-item:first").find(".wp-caption-text:first").html() || el.parents(".gallery-item:first").find(".gallery-caption:first").html() || '';
					}
					if (opt.captions.overrideDesc === false) {
						desc = el.find('img').attr('alt') || '';
					}
				} else if (cont.hasClass('wp-caption') || el.hasClass('wp-caption')) {
					if (opt.captions.overrideTitle === false) {
						title = el.find('img').attr('title') || el.parents(".wp-caption:first").find(".wp-caption-text:first").html() || '';
					}
					if (opt.captions.overrideDesc === false) {
						desc = el.find('img').attr('alt') || '';
					}
					...
					if (opt.captions.overrideTitle === false) {
						title = el.parents(".wp-caption:first").find('img').attr('title') || el.parents(".wp-caption:first").find(".wp-caption-text:first").html() || '';
					}
					if (opt.captions.overrideDesc === false) {
						desc = el.parents(".wp-caption:first").find('img').attr('alt') || '';
					}
				} else if (cont.hasClass('tiled-gallery')) {
					if (opt.captions.overrideTitle === false) {
						title = el.parents(".tiled-gallery-item:first").find(".tiled-gallery-caption").html() || el.find('img').data('image-title') || el.find('img').attr('title') || '';
					}
					if (opt.captions.overrideDesc === false) {
...
(function ($, FooBox) {
    ...
    * @returns {string}
    */
			findSelector: function findSelector(foobox, element) {
				if (!element) {
					return '';
				}
  ...
  */
   ...
   */
		this.load = function (item, container, _success, _error) {
			try {
				...
				if (FooBox.isjQuery(item.content)) {
					...
					if ($.isFunction(_error)) {
						_error('No valid HTML found to display.');
					}
				}
			} catch (err) {
				if ($.isFunction(_error)) {
			...
			if ((item.width == null || item.height == null || item.width == 0 || item.height == 0) && typeof item.selector == 'string') {
				...
				if (FooBox.isjQuery($clone)) {
					...
					if (typeof item.width == 'number' && item.width > 0) {
						...
						item.width = $clone.outerWidth(true) + item.padding;
					}
...
(function ($, FooBox) {
    ...
    * @returns {string}
    */
			findUrl: function findUrl(foobox, element) {
				if (!element) {
					return '';
				}
		...
		this.load = function (item, container, success, error) {
			try {
				...
				if (url.match('//(www.)?youtube|youtu\.be')) {
					...
					item.video_url = 'https://www.youtube.com/embed/' + item.video_id;
...
document.addEventListener('DOMContentLoaded', function () {
    var navMobileToggle = document.querySelector('#js-toggle-mobile-nav'),
        navMobile = document.querySelector('#js-toggle-mobile-menu');

    var searchToggle = document.querySelector('#js-toggle-mobile-search'),
        search = document.querySelector('#js-mobile-search');

    navMobileToggle.addEventListener('click', function (e) {
        if (searchToggle.classList.contains('is-active')) {
            toggleActive(searchToggle, search);
        }

        toggleActive(navMobileToggle, navMobile);

        e.preventDefault();
    });

    searchToggle.addEventListener('click', function (e) {
        if (navMobileToggle.classList.contains('is-active')) {
            toggleActive(navMobileToggle, navMobile);
        }
    ...
    document.querySelectorAll('a.js-remove-hidden').forEach(function (removeLink) {
        removeLink.addEventListener('click', function (e) {
            var item = document.querySelector('#js-hidden-' + removeLink.dataset.item);

            item.querySelectorAll('.is-hidden').forEach(function (hidden) {
                hidden.classList.remove('is-hidden');
            });
			
			item.querySelectorAll('.js-remove-hidden').forEach(function (removeHidden) {
				removeHidden.remove();
			});
...
function toggleActive(toggle, item) {
...
$document.ready(function () {
    ...
    $('#js-nav-search .button').on('click', function (e) {
        var holder = $('#js-nav-search');

        if (!holder.hasClass('has-focus')) {
            holder.addClass('has-focus');
            $('input', holder).focus();
    ...
    if (videos.length && videos.first().css('display') != 'none') {
        isLoaded = true;
...
$window.on('resize', function (e) {
    if (!isLoaded && videos.length && videos.first().css('display') != 'none') {
        isLoaded = true;
...
document.addEventListener('DOMContentLoaded', function () {
    var facultyOpen = document.querySelectorAll('a.js-faculty-open'),
        facultyClose = document.querySelectorAll('a.js-faculty-close');

    if (facultyOpen.length) {
        facultyOpen.forEach(function (open) {
            open.addEventListener('click', function (e) {
                var holder = e.target.closest('div.js-faculty-holder');

                document.querySelectorAll('div.js-faculty-holder.is-active').forEach(function (holder) {
                    holder.classList.remove('is-active');
                });

                holder.classList.add('is-active');

                var offset = window.innerWidth < 768 ? holder.offsetTop : document.querySelector('.js-faculty-listing-holder').offsetTop;
        ...
        facultyClose.forEach(function (close) {
            close.addEventListener('click', function (e) {
                e.target.closest('div.js-faculty-holder').classList.remove('is-active');
...
$window.on('load', function () {
    ...
    if ($('div.js-slider').length != 0) {
        ...
        $.each($('div.js-slider'), function (i, e) {
            slider = $(this);
            wraps['s' + i] = slider.closest('div.js-slider-wrap');
            wraps['s' + i].data('index', 's' + i);

            auto = slider.data('auto') == true ? true : false;
            pager = slider.data('pager') == true ? true : false;
            
            // Changed on 2/2/2021 by Nic Hubbard
            // We were having issues in Chrome with clicks being prevented inside of bxSliders. Removing the touch option
            // for non-touch devices solved the issue.
            if (('ontouchstart' in window) || (navigator.MaxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0)) {
                touch = true;
            } else {
                touch = false;
            }
			//touch = slider.data('touch') == true ? true : false;
            
            delay = slider.data('delay') ? slider.data('delay') : 0;
            mode = slider.data('mode') ? slider.data('mode') : 'horizontal';

            sliders['s' + i] = slider.bxSlider({
                auto: auto,
                mode: mode,
                pause: 5000,
                pager: pager,
                ...
                onSliderLoad: function onSliderLoad(index) {
                    if (slider.getSlideCount() > 1) {
                        $('.slide-controls', wraps['s' + i]).removeClass('is-hidden');
                    }

                    var video = $('video.is-loaded', $('.slide', wraps['s' + i]).eq(index + 1));

                    if (video.length > 0) {
                        video[0].play();
                    }

                    $('.js-slide-caption', wraps['s' + i]).html($('[data-caption]', $('.slide:not(.bx-clone):eq(' + index + ')', wraps['s' + i])).html());
                    wraps['s' + i].removeClass('is-loading');
                },
                onSlideBefore: function onSlideBefore() {
                    $('.js-slide-caption', wraps['s' + i]).addClass('is-loading');
                },
                onSlideAfter: function onSlideAfter(slide) {
                    $('.js-slide-caption', wraps['s' + i]).html($('[data-caption]', slide).html());
                    $('.js-slide-caption', wraps['s' + i]).removeClass('is-loading');

                    var video = $('video.is-loaded', $(slide));
    ...
    $('.js-pager-prev').click(function (e) {
        var pager = $(this),
            wrap = pager.closest('div.js-slider-wrap'),
            slider = sliders[wrap.data('index')];

        if (!pager.hasClass('js-disabled')) {
            pager.addClass('js-disabled');
    ...
    $('.js-pager-next').click(function (e) {
        var pager = $(this),
            wrap = pager.closest('div.js-slider-wrap'),
            slider = sliders[wrap.data('index')];
...
window.initMap = function () {
    ...
    var center = new google.maps.LatLng(38.569875, -122.439489);
    ...
    google.maps.event.addListenerOnce(map, 'idle', function(){
        ...
        var a = url.searchParams.get("t");
        if (c && a) {
            var l = c.split(',');
            var center = new google.maps.LatLng(l[0], l[1]);
            window.infoWindow.setContent(document.getElementById('js-tooltip-' + a).innerHTML);
            var map = window.map;
            var zoom = 19;
            map.setZoom(zoom);
            ...
            $('html, body').animate({
                scrollTop: $('#js-map').position().top - 140
            });

        }
    });

    Array.from(document.getElementsByClassName('js-map-location')).forEach(function (element) {
        google.maps.event.addDomListener(element, 'click', function (event) {
            var location = element.getAttribute('data-location').split(','),
                zoom = element.getAttribute('data-zoom') ? parseInt(element.getAttribute('data-zoom')) : 17;

            center = new google.maps.LatLng(location[0], location[1]);

            infoWindow.setContent(document.getElementById('js-tooltip-' + element.getAttribute('data-tooltip')).innerHTML);
